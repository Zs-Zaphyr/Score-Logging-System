...
void loadSettings() {
	...
	while(1) {
		...
		if(strcmp(cuProLine,"SRT")==0) { //sorting is enabled
			switch(value) {
				case 0: {
					settings.sort.enabled=false;
					break;
		...
		if(strcmp(cuProLine,"SRTA")==0 && settings.sort.enabled==true) { //defaultType
    ...
    if(settings.debugMode==true)
    {
    	...
    	{	
    		if(buckets[i].head==NULL)
    			printf("[BucketSorting]%d-NULL\n",i);
    		else {
    			t=buckets[i].head;
    			printf("[BucketSorting]-%d",i);
    			while(1)
				{
...
void sorting(){
	int i; 
    if(settings.sort.enabled==false)
        printCSV();
...
void printBucketSorted(){
	int i,j;struct node *t=NULL;
	fclose(stdout);
	freopen("scoreS.csv","w",stdout);
	switch(settings.lang) {
		case 0: {
			freopen("lang/en_us/output-N+S.lang","r",stdin);
			break;
